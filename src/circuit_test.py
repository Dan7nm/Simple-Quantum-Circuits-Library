...
def compare_dynamic_qft(input_state: MultiQubit)->None:
    """
    This function receives a quantum state as a MultiQubit object. It constructs a regualar and dynamic quantum fourier transform circuit and runs the dynamic circuit mutliple times and comapres to the regualar circuit. The expected result is to see the central limit theorem with the number of circuit runs.
    The result is plotted using many subplots.
    ...
    """
    # Number of qubits
    n = input_state.get_number_of_qubits()
    # Number of states:
    N = 2**n

    # init classical register for the circuits
    c_reg1 = ClassicalRegister(n)
    c_reg2 = ClassicalRegister(n)

    # Construct the regular circuit:
    reg_qft_circ = QuantumCircuit(input_state,c_reg1)
    reg_qft_circ.load_qft_preset()

    # Construct the dynamic circuit:
    dyn_qft_circ = QuantumCircuit(input_state,c_reg2)
    dyn_qft_circ.load_dynamic_qft_preset()

    # How many measurements to perform each time:
    number_of_measurements = [10,100,1000]
    # Number of means to compute for each measurement number
    num_samples = 300

    # Run the regular circuit on the input state:
    q_state = reg_qft_circ.run_circuit()
    ...
    for i, size in enumerate(number_of_measurements, start=2):
        ...
        for _ in range(num_samples):
            resulting_state = dyn_qft_circ.run_many(size)
            probs = np.abs(resulting_state.get_tensor_vector())**2
        ...
        plt.xlabel('Mean Value')
        plt.ylabel('Density')
        print(f"Finished for measurements:{size}")
